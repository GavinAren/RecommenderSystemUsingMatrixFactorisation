import numpy as np
import csv
from sgdTutorialOptimised import ExplicitMF2
...
def try_model(iter):
	MF_SGD = ExplicitMF2(train, 20, learning='sgd', verbose=True)
	iter_array = [iter]
	testSet = readFile('train_100k_withratings_new.csv')
	predictions = MF_SGD.calculate_learning_curve(iter_array, testSet, sorted_User_dict, sorted_Item_dict, learning_rate=0.001)

	return predictions
import time
start = time.time()
new_rating_matrix = try_model(10)
end = time.time()
